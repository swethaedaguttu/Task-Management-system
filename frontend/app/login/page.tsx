...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (status === 401) {
        if (backendError.toLowerCase().includes('invalid credentials') || backendError.toLowerCase().includes('password')) {
          errorMessage = '❌ Invalid email or password. Please check your credentials and try again.';
        } else {
          errorMessage = '❌ Invalid email or password. Please check your credentials and try again.';
        }
        // Dismiss any existing toasts first to show only one message
        toast.dismiss();
        
        // Show invalid login message for longer duration (1 minute)
      ...
      toast.error(
        <div style={{ padding: '2px 0' }}>
          <div style={{ 
            ...
            {status === 500 ? 'Server Error' : status === 0 ? 'Network Error' : 'Error'}
          </div>
          <div style={{ 
            fontSize: '13px', 
            color: '#7f1d1d',
